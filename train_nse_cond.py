...
from utils import *
from modules import UNet, EMA, UNet2PhaseAmp_Cond, UNet2AmpPhase_Cond
import logging
import numpy
...
class JointDiffusion:
    ...
    def sample_image(self, model1, x, amp, model2, x_cond, amp_cond):
        model1.eval()
        model2.eval()
        ...
        with torch.no_grad():
            ...
            for i in tqdm(reversed(range(1, self.noise_steps)), position=0):
                ...
                w, deltax1, deltax2, order_parameter, order_phase = self.coupling_noise_local(x)
                score_cond = model1(x, log_amp.exp(), t, x_cond, amp_cond)
                score_uncond = model1(x, log_amp.exp(), t, zeros_cond, zeros_cond)
                score = score_cond + 0.0 * (score_cond-score_uncond)
                ...
                score_cond = model2(log_amp, order_parameter, t, amp_cond, x_cond)
                score_uncond = model2(log_amp, order_parameter, t, zeros_cond, zeros_cond)
                score = score_cond + 0.0 * (score_cond - score_uncond)
...
def train_joint(args):
    ...
    model1 = UNet2PhaseAmp_Cond(c_in=1, c_out=1,img_size=args.image_size).to(device)
    model2 = UNet2AmpPhase_Cond(c_in=1, c_out=1, img_size=args.image_size).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, _) in enumerate(pbar):
            ...
            mag_seq, phase_seq = map_to_phase(images)
            mag, mag_cond = mag_seq[:,2:3,:,:], mag_seq[:,0:2,:,:]
            phase, phase_cond = phase_seq[:, 2:3, :, :], phase_seq[:, 0:2, :, :]
            ...
            cond_mask = (torch.rand(mag.size(0), device=mag.device) < 0.1).float().view(mag.size(0), 1, 1, 1)
            for sample2 in range(exp_sam):
                ...
                phase_score = model1(x_t1, mag_t1 , t + 1, (1-cond_mask) * phase_cond, (1-cond_mask) * mag_cond)
            ...
            amp_score = model2(log_mag_t1, order_parameter, t+1, (1-cond_mask) * mag_cond, (1-cond_mask) * phase_cond)
        ...
        if epoch%5==0:
            sampled_images_phase, sampled_images_mag = diffusion.sample_image(ema_model1, phase, mag, ema_model2, phase_cond, mag_cond)
...
def visualize_history_prediction(args):
    device = args.device
    setup_logging(args.run_name)
    args.batch_size = 80
    train_dataloader, test_dataloader = get_data(args)
    ...
    with torch.no_grad():
        for sample, (images, _) in enumerate(pbar):
            ...
            mag, mag_cond = mag_seq[:, 2:3, :, :], mag_seq[:, 0:2, :, :]
            phase, phase_cond = phase_seq[:, 2:3, :, :], phase_seq[:, 0:2, :, :]
            sampled_phase, sampled_mag = diffusion.sample_image(model1, phase, mag, model2, phase_cond, mag_cond)

            fake = map_to_image(sampled_mag, sampled_phase)
            true = map_to_image(mag, phase)

            true = (true - true.min()) / (true.max() - true.min() + 1e-6)
            fake = (fake - fake.min()) / (fake.max() - fake.min() + 1e-6)
            images = (images - images.min()) / (images.max() - images.min() + 1e-6)
            save_images(fake[:, 0:1] * 255, os.path.join("noise", args.run_name, f"fake_x.jpg"), one_channel=True,
                        cmap='viridis')
            save_images(fake[:, 1:2] * 255, os.path.join("noise", args.run_name, f"fake_y.jpg"), one_channel=True,
                        cmap='viridis')
            save_images(true[:, 0:1] * 255, os.path.join("noise", args.run_name, f"true_x.jpg"), one_channel=True,
                        cmap='viridis')
            save_images(true[:, 1:2] * 255, os.path.join("noise", args.run_name, f"true_y.jpg"), one_channel=True,
                        cmap='viridis')
            save_images(images[:,0,0:1] * 255, os.path.join("noise", args.run_name, f"hist_x0.jpg"), one_channel=True,
                        cmap='viridis')
            save_images(images[:,1,0:1] * 255, os.path.join("noise", args.run_name, f"hist_x1.jpg"), one_channel=True,
                        cmap='viridis')
            save_images(images[:,0, 1:2] * 255, os.path.join("noise", args.run_name, f"hist_y0.jpg"), one_channel=True,
                        cmap='viridis')
            save_images(images[:,1, 1:2] * 255, os.path.join("noise", args.run_name, f"hist_y1.jpg"), one_channel=True,
                        cmap='viridis')


def evaluate_dataloaders_mse(args):
    device = args.device
    setup_logging(args.run_name)
...
def launch():
    ...
    args.run_name = "OrientationNSCond"
    args.dataset_name = "ns_seq"
    args.epochs = 300
    args.batch_size = 48
    args.image_size = 128
    args.patch_size = 8
    ...
    args.lr = 1e-4
    train_joint(args)
    visualize_history_prediction(args)
    evaluate_dataloaders_mse(args)
