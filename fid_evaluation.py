...
class FIDEvaluation:
    ...
    def load_or_precalc_dataset_stats(self):
        path = os.path.join(self.stats_dir,  self.dataset+"_stats")
        try:
    ...
    def fid_score_noise(self):
        ...
        for batch in tqdm(batches):
            fake_samples = self.sampler.sample(self.model, batch)
            fake_features = self.calculate_inception_features((fake_samples/torch.pi/0.9).clamp(-1, 1))
            stacked_fake_features.append(fake_features)
        stacked_fake_features = torch.cat(stacked_fake_features, dim=0).cpu().numpy()
        m1 = np.mean(stacked_fake_features, axis=0)
        s1 = np.cov(stacked_fake_features, rowvar=False)

        return print(calculate_frechet_distance(m1, s1, self.m2, self.s2))

    @torch.inference_mode()
    def fid_score_image(self):
        if not self.dataset_stats_loaded:
            self.load_or_precalc_dataset_stats()
        self.model.eval()
        num_batches = int(math.ceil(self.n_samples / self.batch_size))
        ...
        for i, (images, _) in enumerate(pbar):
            ...
            try:
                ...
                break
            real_samples = real_samples.to(self.device)
            fake_samples = self.sampler.sample_image(self.model, real_samples*0.9*torch.pi)
            fake_features = self.calculate_inception_features((fake_samples/torch.pi/0.9).clamp(-1, 1))
            stacked_fake_features.append(fake_features)
        stacked_fake_features = torch.cat(stacked_fake_features, dim=0).cpu().numpy()
        m1 = np.mean(stacked_fake_features, axis=0)
